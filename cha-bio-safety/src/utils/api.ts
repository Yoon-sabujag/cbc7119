...
export const authApi = {
  ...
  updateProfile: (data: { name?: string; phone?: string; email?: string; birthDate?: string | null }) =>
    api.put<{ name: string; phone: string | null; email: string | null; birthDate: string | null }>('/auth/profile', data),
}
...
// ── Phase 25: 화재수신반 원격감시·경보 (panelApi / alarmApi) ──
// 백엔드 /api/panel/* · /api/alarm/* 은 이 디자인 트랙(cbc7119-preview)에는 배포되지 않음
// (별도 데이터 트랙). 모든 consumer 는 react-query try/catch 로 평상시 fallback 하여
// 미배포 404/500 이 트리를 크래시시키지 않도록 처리한다. api 메서드 자체는 정상 throw 유지.
...
export type AlarmEvent = Alarm

// 수신반 상태 (라이브 프레임 신선도 + 에이전트 온라인 + 활성경보 + 정비모드)
...
export const alarmApi = {
  getActive: (): Promise<Alarm | null> => api.get<Alarm | null>('/alarm/active'),
  getEvents: (hours = 48): Promise<AlarmEvent[]> => {
    const qs = new URLSearchParams({ hours: String(hours) })
    return api.get<AlarmEvent[]>('/alarm/events?' + qs)
  },
