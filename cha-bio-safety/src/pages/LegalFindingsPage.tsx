...
function FindingEditModal({ scheduleItemId, finding, onClose }: EditModalProps) {
  const queryClient = useQueryClient()
  ...
  const findingCard = (finding: LegalFinding) => (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={(e) => { e.stopPropagation(); setEditingFinding(finding) }} style={{ fontSize: 10, color: 'var(--t3)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px' }}>수정</button>
