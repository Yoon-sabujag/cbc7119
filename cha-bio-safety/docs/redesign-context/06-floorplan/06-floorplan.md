# 06. 소방 시설 도면 재디자인 컨텍스트

> ⚠️ 2165 라인. 마커 SVG 렌더링 + 점검/자산 모달 인라인. 단계별 진행 권장.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/FloorPlanPage.tsx` |
| 라우트 | `/floorplan` |
| 라인 수 | 2165 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (별도 함수 분기) |
| BottomNav | **숨김** (`/floorplan` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 도면 타입 탭 → 층 선택 → 도면 컨테이너 → 마커 모달 → 자산 배치 모달)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 핀치줌·드래그 패닝의 touch event 핸들링은 비즈니스 로직 → 절대 변경 금지.
- PDF/SVG 마커 렌더링 좌표계 보존. 마커 위치는 정규화된 (x, y) 좌표.
- 도면 위 마커 색은 `status bar` 토큰(`--status-{...}-bar`) 사용.
- 소화기 자산 배치 워크플로우(`?fromMarker=` 동행)는 `ExtinguishersListPage` 와 짝 → 라우팅 파라미터 보존.
...
- 결과는 `src/pages/FloorPlanPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/06-floorplan/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `06-floorplan.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `FloorPlanPage.tsx` | 재디자인 대상 — `src/pages/FloorPlanPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `PdfFloorPlan.tsx` | PDF 도면 렌더러 — `src/components/PdfFloorPlan.tsx` 스냅샷 |
| `SvgFloorPlan.tsx` | SVG 도면 렌더러 (확인 필요) — `src/components/SvgFloorPlan.tsx` 스냅샷 |
| `AccessBlockedPopup.tsx` | 접근불가 팝업 — `src/components/AccessBlockedPopup.tsx` 스냅샷 |
| `InspectionRevisitPopup.tsx` | 재진입 안내 팝업 — `src/components/InspectionRevisitPopup.tsx` 스냅샷 |
| `PhotoButton.tsx` | 사진 버튼 — `src/components/PhotoButton.tsx` 스냅샷 |
| `useInspectionRevisitPopup.ts` | 재진입 팝업 훅 — `src/hooks/useInspectionRevisitPopup.ts` 스냅샷 |
| `usePhotoUpload.ts` | 사진 업로드 훅 — `src/hooks/usePhotoUpload.ts` 스냅샷 |
