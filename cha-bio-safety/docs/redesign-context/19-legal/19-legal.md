...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/LegalPage.tsx` |
| 라우트 | `/legal` |
| 라인 수 | 553 |
| 모바일 | ✅ |
| 데스크톱 | ✅ (좌-라운드 목록 / 우-지적사항 패널) |
| BottomNav | **숨김** (`/legal` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더(탭+연도) → 라운드 카드 리스트 → 데스크톱 우측 FindingsPanel)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 적합/부적합/조건부적합 결과 색은 status 토큰: 적합=safe, 부적합=danger, 조건부=warning, 미입력=text-tertiary.
- 라운드 카드 좌측 색바는 `--status-{...}-bar` 토큰 사용.
- `ResultBadge` 는 ui-index.tsx 에 정의 — 그대로 재사용.
- 데스크톱 좌/우 분할은 50/50 유지 (RemediationPage 와 동일 패턴).
...
- 결과는 `src/pages/LegalPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/19-legal/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `19-legal.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `LegalPage.tsx` | 재디자인 대상 — `src/pages/LegalPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `ui-index.tsx` | 공통 UI (`ResultBadge` 등) — `src/components/ui/index.tsx` 스냅샷 |
| `PhotoGrid.tsx` | 사진 그리드 — `src/components/PhotoGrid.tsx` 스냅샷 |
| `PhotoSourceModal.tsx` | 사진 소스 모달 — `src/components/PhotoSourceModal.tsx` 스냅샷 |
| `useMultiPhotoUpload.ts` | 다중 사진 업로드 훅 — `src/hooks/useMultiPhotoUpload.ts` 스냅샷 |
