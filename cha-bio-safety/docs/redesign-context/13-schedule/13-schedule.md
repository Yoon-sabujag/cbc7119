# 13. 월간 점검 계획 재디자인 컨텍스트

> ⚠️ 1062 라인. 캘린더+미리보기+등록 모달+자동 생성 인라인.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/SchedulePage.tsx` |
| 라우트 | `/schedule` |
| 라인 수 | 1062 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/schedule` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 월/연도 네비게이션 → 캘린더 그리드 → 일자별 일정 리스트 → 등록 모달 → 월간 미리보기)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 월간 캘린더 7x6 그리드는 grid utility 로 표현 가능. 셀 hover/active 상태는 surface 토큰.
- 카테고리 5종 dot 색은 별도 카테고리 색 시스템 (status 토큰과 별개). 라이트 모드 호환만 확인.
- 멀티데이 일정 범위 매칭은 비즈니스 로직 → 보존.
- 자동 생성(`generateMonthlyPlan`)은 별도 util → UI는 트리거 버튼만 손보면 됨.
...
- 결과는 `src/pages/SchedulePage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/13-schedule/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `13-schedule.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `SchedulePage.tsx` | 재디자인 대상 — `src/pages/SchedulePage.tsx` 스냅샷 |
