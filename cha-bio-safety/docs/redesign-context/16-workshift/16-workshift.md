...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/WorkShiftPage.tsx` |
| 라우트 | `/workshift` |
| 라인 수 | 226 |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`DESKTOP_HEADER_HIDE_PATHS` — App 헤더 숨기고 자체 헤더, padding-top 12vh) |
| BottomNav | **숨김** (`/workshift` ∈ `MOBILE_NO_NAV_PATHS`) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 연/월 네비 → 매트릭스 컨테이너(좌측 고정 + 우측 스크롤) → 셀 → 범례)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 당/비/주/휴 셀 색은 `duty` 토큰(`--duty-day/-night/-off/-leave`) 사용 — `status` 토큰과 혼용 금지.
- 좌측 고정 이름 컬럼 + 가로 스크롤 날짜 매트릭스는 `position: sticky` 패턴.
- 공휴일 빨강은 `status-danger` 가 아닌 캘린더 빨강(별도 처리) — 의미 차이 주의.
...
- 결과는 `src/pages/WorkShiftPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/16-workshift/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `16-workshift.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `WorkShiftPage.tsx` | 재디자인 대상 — `src/pages/WorkShiftPage.tsx` 스냅샷 |
