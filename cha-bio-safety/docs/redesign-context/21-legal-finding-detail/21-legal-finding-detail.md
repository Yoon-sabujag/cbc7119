...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/LegalFindingDetailPage.tsx` |
| 라우트 | `/legal/:id/finding/:fid` |
| 라인 수 | 279 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/legal/...` 패턴 매칭, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → KVRow 정보 → 사진 그리드 → 조치 입력 → 다운로드 + 고정 하단 CTA)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 자체 헤더 스타일은 RemediationDetailPage(05) 와 동일 패턴으로 통일 권장.
- KVRow + Section 헤더는 ui-index.tsx 의 컴포넌트 재사용.
- 다중 사진 업로드 hook 보존.
...
- 결과는 `src/pages/LegalFindingDetailPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/21-legal-finding-detail/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `21-legal-finding-detail.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `LegalFindingDetailPage.tsx` | 재디자인 대상 — `src/pages/LegalFindingDetailPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `ui-index.tsx` | 공통 UI (`KVRow` 등) — `src/components/ui/index.tsx` 스냅샷 |
