...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/LegalFindingsPage.tsx` |
| 라우트 | `/legal/:id` |
| 라인 수 | 643 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 지적사항 카드 리스트 → 신규 등록 BottomSheet (zone/층/위치/항목/설명/다중 사진))
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 점검 항목 picker(`FINDING_ITEMS` 19종)는 비즈니스 로직 → 항목 변경 금지, UI만 재설계.
- BottomSheet 패턴은 다른 페이지(CheckpointsPage, StaffManagePage)와 통일 권장.
- 다중 사진 업로드(`useMultiPhotoUpload`) 는 단일 사진 업로드와 별개 패턴 — 보존.
...
- 결과는 `src/pages/LegalFindingsPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/20-legal-findings/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `20-legal-findings.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `LegalFindingsPage.tsx` | 재디자인 대상 — `src/pages/LegalFindingsPage.tsx` 스냅샷 |
