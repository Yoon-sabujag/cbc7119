...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/CctvInfoPage.tsx` |
| 라우트 | `/cctv` |
| 라인 수 | 69 |
| 모바일 | ✅ (1열) |
| 데스크톱 | ✅ (2열, max-width 960) |
| BottomNav | **모바일 숨김** (App.tsx `isCctv` 분기 — 헤더 좌측에 뒤로가기, 메뉴는 `desktopOnly`) |
| SideMenu | 데스크톱 전용 (모바일 항목 `desktopOnly: true`) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 페이지 컨테이너 → DVR 카드 그리드 → 카드 헤더(채널+배지) → 포트 그리드 → 출처 푸터)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 정적 페이지(상태 없음). API 호출이 없는 페이지 → 인터랙션 처리 없음.
- 보존기간 "추정/확정" 배지: 추정 = `info`, 확정 = `safe` 토큰 권장.
- HDD 용량 숫자는 `text-mono` 클래스 + `tabular-nums` 로 자릿수 정렬 권장.
...
- 결과는 `src/pages/CctvInfoPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/10-cctv-info/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `10-cctv-info.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `CctvInfoPage.tsx` | 재디자인 대상 — `src/pages/CctvInfoPage.tsx` 스냅샷 |
