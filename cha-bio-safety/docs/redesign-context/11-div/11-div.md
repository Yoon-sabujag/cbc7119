# 11. DIV 압력 관리 재디자인 컨텍스트

> ⚠️ 1136 라인. 4탭 + 측정점 차트 + 모달 인라인.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/DivPage.tsx` |
| 라우트 | `/div` |
| 라인 수 | 1136 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`DESKTOP_HEADER_HIDE_PATHS` — App 헤더 숨기고 자체 헤더) |
| BottomNav | **숨김** (`/div` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 탭+연도 필터 → 측정점 카드(차트 포함) → 입력 모달 3종)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 압력 트렌드 SVG 차트는 비즈니스 로직 → 차트 자체는 보존, 컨테이너/축/색만 토큰화.
- 이상값 색은 status 토큰 사용 (`±10%` = warning, `±20%` = danger).
- DIV/컴프레셔 1:1 매핑(메모리에 명시된 룰)은 데이터 모델에 이미 반영 — UI에서 변경 안 함.
- 숫자(1차압/2차압/챔버압) 표시는 `text-mono` + `tabular-nums` 로 자릿수 정렬 권장.
...
- 결과는 `src/pages/DivPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/11-div/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `11-div.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `DivPage.tsx` | 재디자인 대상 — `src/pages/DivPage.tsx` 스냅샷 |
