...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/NotFoundPage.tsx` |
| 라우트 | `*` |
| 라인 수 | 11 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | 라우팅에 따라 다름 (인증 후 진입 시 노출, 인증 전이면 숨김) |
| SideMenu | 라우팅에 따라 다름 |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 페이지 컨테이너(센터링) → 404 큰 텍스트 → 안내 문구 → 대시보드 이동 버튼)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 가장 단순한 페이지. 디자인 토큰 적용 연습으로 적합.
- "404" 큰 텍스트는 `text-display` (28px) 보다 더 큰 디스플레이 (예: 96px) 가 일반적 — 인라인 fontSize 허용.
- 안내 문구 `text-secondary` + 액센트 버튼.
...
- 결과는 `src/pages/NotFoundPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/30-not-found/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `30-not-found.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `NotFoundPage.tsx` | 재디자인 대상 — `src/pages/NotFoundPage.tsx` 스냅샷 |
