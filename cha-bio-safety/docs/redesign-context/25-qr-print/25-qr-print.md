...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/QRPrintPage.tsx` |
| 라우트 | `/qr-print` |
| 라인 수 | 330 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/qr-print` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 카테고리 셀렉터 → canvas 미리보기 → 다운로드 CTA)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- jsPDF + qrcode 클라이언트 생성 로직은 비즈니스 로직 → 보존.
- 카테고리별 카드 사이즈/레이아웃은 종이 인쇄용 → 변경 금지 (디자인 시스템 토큰 적용 영역 아님).
- 페이지 화면(셀렉터, 미리보기 컨테이너)만 토큰화.
...
- 결과는 `src/pages/QRPrintPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/25-qr-print/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `25-qr-print.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `QRPrintPage.tsx` | 재디자인 대상 — `src/pages/QRPrintPage.tsx` 스냅샷 |
