...
> ⚠️ 5346 라인. 카테고리별 모달 다수 인라인 구현. 단계별 진행 필수 (한 번에 다 시도 금지).
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/InspectionPage.tsx` |
| 라우트 | `/inspection` |
| 라인 수 | 5346 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`DesktopInspectionView` 동일 파일 내 분기) |
| BottomNav | 모바일 노출 ✅ |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 카테고리 그리드 → Zone/층 탭 → CP 리스트/휠피커 → 결과 모달들 → 사진/메모 영역)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침 (5346 라인):**
- 한 번의 응답으로 전체를 다시 쓰려 하지 말고 위 단계 1번부터 끊어서. 한 단계씩 사용자 확인 후 다음 단계.
- 카테고리별 모달은 패턴이 미세하게 다르므로 공통 모달 컴포넌트(`<InspectionModal>` 등) 추출 제안 권장 — 단, 추출은 별도 단계로 분리해 비즈니스 로직 보존을 검증해가며 진행.
- DIV/컴프 1:1 매핑, isCpCompleted 룰 등 메모리에 명시된 비즈니스 룰 절대 변경 금지.
...
- 결과는 `src/pages/InspectionPage.tsx` 교체본으로 출력 (또는 단계별 부분 교체)
...
이 폴더(`docs/redesign-context/02-inspection/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `02-inspection.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `InspectionPage.tsx` | 재디자인 대상 — `src/pages/InspectionPage.tsx` 스냅샷 (5346 라인) |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `ui-index.tsx` | 공통 UI 컴포넌트 (`StatusBadge`, `CatBar` 등) — `src/components/ui/index.tsx` 스냅샷 |
