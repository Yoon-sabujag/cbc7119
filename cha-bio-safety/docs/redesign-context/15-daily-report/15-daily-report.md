...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/DailyReportPage.tsx` |
| 라우트 | `/daily-report` |
| 라인 수 | 840 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/daily-report` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더(날짜 네비) → 자동 집계 카드 섹션 → 수동 입력 textarea → 다운로드 액션)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 디바운스 자동 저장 로직(`debounceRef`)은 비즈니스 로직 → 보존.
- 자동 데이터 카드와 수동 입력 textarea는 별도 카드로 분리.
- 미래 날짜 비활성화 UX는 `text-disabled` 토큰 사용.
...
- 결과는 `src/pages/DailyReportPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/15-daily-report/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `15-daily-report.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `DailyReportPage.tsx` | 재디자인 대상 — `src/pages/DailyReportPage.tsx` 스냅샷 |
