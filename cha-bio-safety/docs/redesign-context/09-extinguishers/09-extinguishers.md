...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/ExtinguishersListPage.tsx` |
| 라우트 | `/extinguishers (`?fromMarker=` 쿼리: 도면에서 자산 배치 동행 모드)` |
| 라인 수 | 1168 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`useIsDesktop` 분기) |
| BottomNav | 모바일 노출 ✅ (단, App.tsx `isExtinguishers` 분기로 헤더 좌측에 뒤로가기 추가 주입) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 필터 바 → 카드 리스트(expand 가능) → 등록/수정 모달 → 4종 확인 모달)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- `?fromMarker=` 동행 모드 워크플로우(도면→자산 배치)는 비즈니스 로직 → 보존.
- 4종 확인 모달(`confirmDelete/Dispose/Unassign/Swap`)을 공통 `<ConfirmModal>` 추출 권장 (별도 단계).
- GlobalHeader portal slot 패턴 보존.
- 교체 경고 배지는 `status warning` 토큰 사용.
...
- 결과는 `src/pages/ExtinguishersListPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/09-extinguishers/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `09-extinguishers.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `ExtinguishersListPage.tsx` | 재디자인 대상 — `src/pages/ExtinguishersListPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `GlobalHeader.tsx` | 헤더 portal slot 패턴 참고용 — `src/components/GlobalHeader.tsx` 스냅샷 |
