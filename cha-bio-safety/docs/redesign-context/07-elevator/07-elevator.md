...
> ⚠️ 3209 라인. 6개 탭 + 5개 모달 인라인. 데스크톱은 자체 헤더 (App.tsx 헤더 숨김). 단계별 진행 필수.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/ElevatorPage.tsx` |
| 라우트 | `/elevator (URL 파라미터 `?tab=fault`, `?modal=fault_new` 등 지원)` |
| 라인 수 | 3209 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`DESKTOP_HEADER_HIDE_PATHS` — App 헤더 숨기고 자체 헤더) |
| BottomNav | 모바일 노출 ✅ |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더(탭+검색) → 호기 카드 그리드 → 5개 모달 → 호기 상세 타임라인 → KOELSA 이력)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 6개 탭은 같은 페이지에서 토글 → 탭별로 별도 컴포넌트 분리는 별도 단계로.
- KOELSA 이력 카드는 `KoelsaHistorySection` 으로 이미 분리됨 → 그 컴포넌트만 재디자인하는 단계 권장.
- URL 파라미터(`?tab=`, `?modal=`) 처리는 비즈니스 로직 → 보존.
- TKE 자동 연결은 비즈니스 로직 → 변경 금지.
...
- 결과는 `src/pages/ElevatorPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/07-elevator/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `07-elevator.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `ElevatorPage.tsx` | 재디자인 대상 — `src/pages/ElevatorPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `KoelsaHistorySection.tsx` | KOELSA 이력 카드 — `src/components/KoelsaHistorySection.tsx` 스냅샷 |
| `PdfFloorPlan.tsx` | PDF 렌더러 (검사성적서) — `src/components/PdfFloorPlan.tsx` 스냅샷 |
| `PhotoButton.tsx` | 사진 버튼 — `src/components/PhotoButton.tsx` 스냅샷 |
| `PhotoSourceModal.tsx` | 사진 소스 선택 모달 — `src/components/PhotoSourceModal.tsx` 스냅샷 |
