...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/ElevatorFindingDetailPage.tsx` |
| 라우트 | `/elevator/findings/:fid` |
| 라인 수 | 503 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/elevator/findings` 패턴 매칭, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → KVRow 정보 영역 → 이미지 뷰어 → 조치 입력 폼 → 고정 하단 CTA)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 이미지 뷰어 핀치줌/드래그/더블탭은 비즈니스 로직 → 보존.
- 자체 헤더 + 고정 하단 CTA 패턴은 RemediationDetailPage 와 동일 → 같은 패턴 사용.
- KVRow 컴포넌트(`ui-index.tsx`)는 그대로 사용.
...
- 결과는 `src/pages/ElevatorFindingDetailPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/08-elevator-finding-detail/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `08-elevator-finding-detail.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `ElevatorFindingDetailPage.tsx` | 재디자인 대상 — `src/pages/ElevatorFindingDetailPage.tsx` 스냅샷 |
