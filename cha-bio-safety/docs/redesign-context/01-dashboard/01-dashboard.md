...
> 이 파일은 자기-완결적입니다. 새 채팅에 이 한 파일만 복붙하면 (+ 섹션 5의 첨부 파일들) 작업 시작 가능.
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/DashboardPage.tsx` |
| 라우트 | `/dashboard` |
| 라인 수 | 697 |
| 모바일 | ✅ (5행 그리드 단일 컬럼) |
| 데스크톱 | ✅ (좌: 점검현황+빠른도구 / 우: 미니 캘린더+오늘 일정) |
| BottomNav | 모바일 노출 ✅ |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (대시보드의 경우: 통계카드 → 점검항목카드 → 빠른도구 → 일정 → 도넛)
...
- 결과는 `src/pages/DashboardPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/01-dashboard/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `01-dashboard.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `DashboardPage.tsx` | 재디자인 대상 — `src/pages/DashboardPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `ui-index.tsx` | 공통 UI 컴포넌트 (`DutyChip`, `RoleLabel`, `Donut`, `StatusBadge`, `CatBar`) — `src/components/ui/index.tsx` 스냅샷 |

> 모두 스냅샷이므로 작업 도중 원본이 바뀌어도 영향 없음. 작업 시작이 며칠 이상 미뤄지면 재생성 권장.
>
> 추가 권장: lucide-react는 이미 설치되어 있고, 커스텀 SVG 아이콘은 `src/components/ui/icons.tsx` 에 정의되어 있습니다 (`StairsIcon`, `ShutterIcon`, `ExitSignIcon`, `SmokeVentIcon`, `HoseReelIcon`, `FireExtinguisherCustom`). 대시보드에서 직접 쓰지는 않지만 알아두시면 다른 페이지 작업 시 도움.
