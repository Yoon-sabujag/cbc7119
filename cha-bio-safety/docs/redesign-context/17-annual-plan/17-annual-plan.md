...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/AnnualPlanPage.tsx` |
| 라우트 | `/annual-plan` |
| 라인 수 | 225 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/annual-plan` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 미리보기 이미지 + 연도 오버레이 → 캘리브 모드 토글 → 다운로드 CTA)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 캘리브 모드 좌표 보정은 비즈니스 로직 → 보존.
- 미리보기 이미지 위에 절대 위치 오버레이 패턴 보존.
- 다운로드 버튼은 fixed bottom CTA.
...
- 결과는 `src/pages/AnnualPlanPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/17-annual-plan/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `17-annual-plan.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `AnnualPlanPage.tsx` | 재디자인 대상 — `src/pages/AnnualPlanPage.tsx` 스냅샷 |
