...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/ReportsPage.tsx` |
| 라우트 | `/reports` |
| 라인 수 | 405 |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`useIsDesktop` 분기) |
| BottomNav | **숨김** (`/reports` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 연도/월 셀렉터 → 보고서 카드 그리드 → 미리보기 패널 → 다운로드 버튼)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- xlsx 템플릿 패칭, 점검자 자동 채움 등 비즈니스 로직 → 보존.
- 보고서 카드 그리드: 데스크톱 3-4열 / 모바일 1-2열 권장.
- 미리보기는 `ExcelPreview` 컴포넌트 그대로 사용.
...
- 결과는 `src/pages/ReportsPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/14-reports/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `14-reports.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `ReportsPage.tsx` | 재디자인 대상 — `src/pages/ReportsPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `ExcelPreview.tsx` | Excel 미리보기 컴포넌트 — `src/components/ExcelPreview.tsx` 스냅샷 |
