...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/DocumentsPage.tsx` |
| 라우트 | `/documents` |
| 라인 수 | 162 |
| 모바일 | ✅ (탭 전환) |
| 데스크톱 | ✅ (좌우 2단 동시 표시, max 1200px) |
| BottomNav | 모바일 노출 ✅ |
| SideMenu | 모바일 햄버거 메뉴 사용 가능 |
| 접근 권한 | 업로드/삭제는 **admin 전용** |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 페이지 컨테이너 → 모바일 탭 / 데스크톱 2단 분기 → DocumentSection 카드 → DocumentUploadForm 모달/시트)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 페이지 자체는 컨테이너만(162 라인). 실제 디자인 변경은 `DocumentSection` 과 `DocumentUploadForm` 에서 더 많이 일어남 → 두 컴포넌트도 같이 재디자인 필요.
- multipart 업로드 진행률, abort/retry, beforeunload guard는 비즈니스 로직 → 보존.
- 모바일 탭 전환 / 데스크톱 좌우 2단은 lg:* 분기 정당화.
...
- 결과는 `src/pages/DocumentsPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/22-documents/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `22-documents.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `DocumentsPage.tsx` | 재디자인 대상 — `src/pages/DocumentsPage.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `DocumentSection.tsx` | 문서 섹션 카드 (이 페이지 핵심) — `src/components/DocumentSection.tsx` 스냅샷 |
| `DocumentUploadForm.tsx` | 업로드 폼 (이 페이지 핵심) — `src/components/DocumentUploadForm.tsx` 스냅샷 |
