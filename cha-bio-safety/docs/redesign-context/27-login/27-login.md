...
| 라인 수 | 220 |
| 모바일 | ✅ |
| 데스크톱 | ✅ (`useMediaQuery('(min-width: 768px)')` 분기) |
| BottomNav | **숨김** (`/login` ∈ `MOBILE_NO_NAV_PATHS` ∩ `DESKTOP_NO_NAV_PATHS` — 인증 전 페이지) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 페이지 컨테이너 → 로고/타이틀 → 사번 입력 → 비밀번호 입력 → 직원 카드 그리드 → 로그인 CTA → 안내 문구)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 인증 전 페이지 — 디자인 토큰은 사용하되 다크/라이트 둘 다 깔끔해야 함.
- 직원 카드 그리드 색상(`CARD_COLORS` 6종)은 status/duty 토큰과 별개의 카테고리 색 — 그대로 유지.
- 그라디언트 로그인 버튼은 액센트 토큰 기반으로 단순화 권장 (강한 그라디언트보다는 solid + hover).
...
- 결과는 `src/pages/LoginPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/27-login/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `27-login.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `LoginPage.tsx` | 재디자인 대상 — `src/pages/LoginPage.tsx` 스냅샷 |
