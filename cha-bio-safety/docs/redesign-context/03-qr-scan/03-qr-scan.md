...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/QRScanPage.tsx` |
| 라우트 | `/inspection/qr` |
| 라인 수 | 297 |
| 모바일 | ✅ (주 사용 환경 — 카메라 스캔) |
| 데스크톱 | ✅ (구조상 동작하나 카메라 의존) |
| BottomNav | 모바일 노출 ✅ |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 카메라 영역 → 권한/에러 alert → 수동 입력 폼 → 헤더 토글 버튼)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- `html5-qrcode` 가 직접 DOM 조작하는 영역(`#qr-reader-region`)은 라이브러리 요구사항이 우선 — 컨테이너 dimension/position만 재설계.
- iOS Ultra Wide 카메라 선택, zoom 0.5x 강제 등 카메라 셋업 로직은 비즈니스 로직 → 절대 변경 금지.
- 카메라/수동 토글 버튼은 GlobalHeader portal 로 주입되는 패턴 유지.
...
- 결과는 `src/pages/QRScanPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/03-qr-scan/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `03-qr-scan.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `QRScanPage.tsx` | 재디자인 대상 — `src/pages/QRScanPage.tsx` 스냅샷 |
