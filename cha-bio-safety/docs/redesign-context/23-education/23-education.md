...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/EducationPage.tsx` |
| 라우트 | `/education` |
| 라인 수 | 591 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/education` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 직원 카드 리스트 → 등록 모달)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- D-day 배지 색 분기 (>30일=safe, 0~30일=warning, <0=danger) — status 토큰 매핑.
- D-day 계산은 비즈니스 로직 → 보존.
- 직원 카드는 카드 내부 KVRow 패턴.
...
- 결과는 `src/pages/EducationPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/23-education/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `23-education.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `EducationPage.tsx` | 재디자인 대상 — `src/pages/EducationPage.tsx` 스냅샷 |
