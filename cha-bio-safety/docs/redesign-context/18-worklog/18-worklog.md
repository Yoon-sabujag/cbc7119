# 18. 업무 수행 기록표 재디자인 컨텍스트

> ⚠️ 1216 라인. admin 전용. 폼/미리보기/Excel 인라인.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/WorkLogPage.tsx` |
| 라우트 | `/worklog` |
| 라인 수 | 1216 ⚠️ |
| 모바일 | ✅ |
| 데스크톱 | ✅ (좌-폼 / 우-미리보기 패널) |
| BottomNav | **숨김** (`/worklog` ∈ `MOBILE_NO_NAV_PATHS`, 자체 헤더 사용) |
| SideMenu | 자체 헤더 사용으로 미노출 |
| 접근 권한 | **admin 전용** — 비-admin 진입 처리는 코드 상단만 봐서는 불명확 |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 자체 헤더 → 월 선택 → 좌-폼(4개 카드 섹션) → 우-미리보기 → 다운로드 CTA)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- admin 권한 체크는 비즈니스 로직 → 보존.
- 인라인 스타일 상수(`iconBtn`/`navBtn`/`smallBtn`)는 새 디자인에서 유틸 클래스로 대체.
- 4개 카드 섹션은 동일 패턴 → 공통 `<WorkLogSection>` 추출 가능.
- 미리보기 패널은 데스크톱 전용 분할.
...
- 결과는 `src/pages/WorkLogPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/18-worklog/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `18-worklog.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `WorkLogPage.tsx` | 재디자인 대상 — `src/pages/WorkLogPage.tsx` 스냅샷 |
