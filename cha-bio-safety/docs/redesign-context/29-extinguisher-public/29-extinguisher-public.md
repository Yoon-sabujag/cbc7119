# 29. 소화기 공개 페이지 재디자인 컨텍스트

> 주의: 인쇄(종이 양식) 친화 페이지. 디자인 시스템 토큰을 일부만 적용 (페이지 컨테이너/배경만), 표 자체는 종이 양식 그대로 유지.
>
...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/ExtinguisherPublicPage.tsx` |
| 라우트 | `/e/:checkpointId (인증 없이 접근 가능)` |
| 라인 수 | 149 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | 인증 전 페이지 — 미노출 |
| SideMenu | 인증 전 페이지 — 미노출 |
| 접근 권한 | **인증 없음** (App.tsx에서 `<Auth>` 미감싸기, 미들웨어 public route) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 페이지 컨테이너만 토큰화 → 표 자체는 보존)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- **이 페이지는 종이 인쇄 양식을 모방하는 게 목적**. 디자인 시스템을 강하게 적용하지 말 것.
- 흰 배경/검정 텍스트/빨강 헤더는 인쇄용 의도된 색 — 변경 금지.
- 토큰화는 페이지 외곽(혹시 모바일에서 화면으로 볼 때 컨테이너 폭/패딩)만 적용.
- 표 자체는 단순 HTML 그대로 유지.
...
- 결과는 `src/pages/ExtinguisherPublicPage.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/29-extinguisher-public/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `29-extinguisher-public.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `ExtinguisherPublicPage.tsx` | 재디자인 대상 — `src/pages/ExtinguisherPublicPage.tsx` 스냅샷 |
