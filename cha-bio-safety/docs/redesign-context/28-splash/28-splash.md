...
| 항목 | 값 |
|---|---|
| 파일 경로 | `src/pages/SplashScreen.tsx` |
| 라우트 | `/` |
| 라인 수 | 83 |
| 모바일 | ✅ |
| 데스크톱 | ✅ |
| BottomNav | **숨김** (`/` ∈ `MOBILE_NO_NAV_PATHS` ∩ `DESKTOP_NO_NAV_PATHS`) |
...
2. 큰 컴포넌트부터 작은 컴포넌트 순서로 (이 페이지의 경우: 전체 컨테이너 → 로고 영역 → 진행바 + 텍스트 → 버전 라벨 → InstallPrompt)
3. 인터랙션과 상태 처리(loading/error/empty)는 마지막
4. 한 번에 다 하지 말고 단계별로 (stream 끊김 방지)

**이 페이지 특수 지침:**
- 진행바 그라디언트는 액센트 토큰 단색으로 단순화 검토 (현재 #3b82f6→#0ea5e9 = accent → info).
- 버전 라벨은 `text-tertiary` 톤.
- 진행 텍스트(시스템 초기화/...)는 `text-secondary`.
- `InstallPrompt` 는 별도 컴포넌트.
...
- 결과는 `src/pages/SplashScreen.tsx` 교체본으로 출력
...
이 폴더(`docs/redesign-context/28-splash/`) 안의 모든 파일을 새 채팅에 함께 드래그·첨부하면 됩니다.

| 파일 | 역할 |
|---|---|
| `28-splash.md` | 이 컨텍스트 문서 (지금 보고 있는 이 파일) |
| `design-system.md` | **권위 있는 디자인 시스템 스펙 v0.1.1** (Progress Color Rule, Stat Card 룰, Iconography 등 포함) — `docs/design-system.md` 스냅샷 |
| `SplashScreen.tsx` | 재디자인 대상 — `src/pages/SplashScreen.tsx` 스냅샷 |
| `tokens.css` | 디자인 토큰 — `src/styles/tokens.css` 스냅샷 |
| `typography.css` | 타이포그래피 — `src/styles/typography.css` 스냅샷 |
| `InstallPrompt.tsx` | PWA 설치 안내 컴포넌트 — `src/components/InstallPrompt.tsx` 스냅샷 |
