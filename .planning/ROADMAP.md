...
### Ad-hoc: 도면 4종 완성 + 소화기 관리 + 인라인 점검 (completed 2026-04-03, GSD 외부 작업)

- **도면 4종 완성**: 감지기(연기/열 2종 마커), 스프링클러(폐쇄형헤드/개방형헤드/헤드왕/시험밸브 4종), 소화기·소화전(소화기/소화전/완강기/DIV 4종). PDF 크롭 → pymupdf 4000px 투명 PNG 파이프라인 확립 (도면PNG변환_가이드.md 참조).
- **소화기 관리 데이터**: extinguishers 테이블(migration 0035), 엑셀 관리대장 448개 이관 (종류/형식승인/제조년월/제조업체/접두문자/증지번호/제조번호/비고).
- **소화기 점검 UI**: 피커에 관리번호+종류 뱃지, 상세정보 패널, 연한 3단계 경고(도래 9년 노랑/임박 9.5년 주황/초과 10년 빨강).
- **소화기 리스트 오버레이**: 점검 모달 내 풀스크린, 종류별 요약 뱃지, 연한도래/임박/초과 필터 버튼, 제조번호 검색, 구역/층 필터, 탭 상세 펼침.
- **마커 편집 권한 확장**: admin + 윤종엽(2022051052), API 3곳 동시 적용.
...
- [x] 08-02-PLAN.md — MealPage 달력 UI + 탭 인터랙션 + 통계 카드 + 메뉴표 placeholder
**UI hint**: yes
...
| 11. Elevator Inspection Certs | v1.1 | 1/3 | In Progress|  |
