...
autonomous: true
requirements: [Q-26-W1]
must_haves:
  truths:
    - "wave-1-index.md 파일이 단일 진입점으로 존재한다 (cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md)"
    - "§1~§8 8-section 헤더가 모두 존재한다 (인벤토리 / sub-wave / design-system fence / components.css inherit / 메모리 룰 / negative / OQ / verify)"
    - "§1 인벤토리 표가 StaffManagePage.tsx 530 lines 4 sub-area (외곽+admin 가드 / 헤더 (모바일/데스크톱 분기) / 직원 카드 리스트 (모바일 카드 + 데스크톱 테이블 + skeleton/error/empty) / 등록·수정 BottomSheet/DesktopModal + 폼 + 비밀번호 초기화 confirm + 비활성화 confirm + ReplaceModal) line 범위 박제"
    - "§2 sub-wave 분배표 3 sub-wave (W2 외곽+admin 가드+헤더 / W3 직원 카드 리스트(모바일+데스크톱 테이블)+skeleton/error/empty+모바일 FAB / W4 BottomSheet+DesktopModal 등록 폼 6+role toggle+confirmReset+confirmDeactivate+ReplaceModalContent) + W5 TSX 변환"
    - "§3 design-system v0.1.1 fence verbatim 7건 (§1.1 / §1.2 / §1.3 / §6.4 / §6.5 / §6.8 / §7.1)"
    - "§5 메모리 룰 inline 인용 ≥10 unique slug"
    - "§6 negative rule ≥10건 (src/** 0 / sketch HTML 0 / wrangler 0 / npm run deploy 0 / staffApi 4 mutation 변경 0 / shift_offset 0~3 4-shift 사이클 변경 0 / shift_fixed 옵션 변경 0 / role 옵션 변경 0 / duty 토큰 매핑 변경 0 / admin 가드 변경 0 / 비밀번호 초기화 confirm 변경 0)"
    - "§7 OQ ≥5 (BottomSheet/DesktopModal 인라인 보존 / CTA 단색 / shift_offset+shift_fixed 폼 UI / 외곽 hex 치환 / 폰트 격상 / Lucide 치환)"
    - "§8 verify gate 8개 (§1~§8 헤더 grep / sub-wave row ≥3 / unique slug ≥10 / OQ ≥5 / src/** 0 / sketch HTML 0 / 비즈 anchor 박스 존재 / fence ≥7 + negative ≥10)"
    - "§1.3 비즈 anchor 박스 박제 (staffApi 4종 (list/create/update/resetPassword) / useMutation 4건 (create/update/resetPw/deactivate) / ReplaceModalContent 의 2× staffApi.update / queryKey ['staff-list'] / shiftOffset 0~3 + shiftFixed 'day'/null / role admin/assistant / rankOfTitle 정렬 (대리/주임/기사) / window.__openReplaceModal 훅 / 폼 필드 8종 / duty 토큰 매핑 / 사번 10자리 검증 / appointedAt 사번 앞 8자리 자동 / 카피 verbatim)"
  artifacts:
    - path: "cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md"
      provides: "redesign/26-staff-manage W2~W4 sketch + W5 TSX 진입을 위한 단일 인덱스 (8-section + 비즈 anchor + OQ)"
      min_lines: 450
  key_links:
    - from: "wave-1-index.md §1"
      to: "StaffManagePage.tsx 530 lines"
      via: "4 sub-area line 범위 박제 + 비즈 anchor 보존"
...
<objective>
redesign/26-staff-manage W1 — `StaffManagePage.tsx` (530 lines, admin 전용 BottomSheet/DesktopModal 패턴, CheckpointsPage(24) 친척) 의 element 인벤토리 + 비즈 anchor 박제 + design-system v0.1.1 fence verbatim + 메모리 룰 inline + negative rule + OQ + verify gate 를 통합한 **단일 진입점 마크다운** 1개 (`wave-1-index.md`) 를 생성한다. W2~W4 후속 sketch wave + W5 TSX 변환 작업자는 이 파일 하나만 읽으면 모든 컨텍스트를 확보한다.

Purpose: 24-checkpoints W1 인덱스 mirror 패턴을 26-staff-manage 에도 적용 (CheckpointsPage 693l 5 sub-area / 4 sub-wave → StaffManagePage 530l 4 sub-area / 3 sub-wave 로 축소 — 19 카테고리/유도등/소화기 분기/'basement'='common' eq 등 복잡도 없음). 다음 wave (W2~W4 sketch + W5 TSX) 의 컨텍스트 낭비 차단 + 비즈 anchor (admin 가드 + staffApi 4종 + useMutation 4건 + ReplaceModalContent + 직급 정렬 + 사번 10자리 검증 + appointedAt 자동 채움 + duty 토큰 매핑 + window.__openReplaceModal 훅) 1 byte 변경 사고 사전 예방.

Output: `cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md` 단일 파일 (450~600 lines 예상, dul 622l 보다 축소).
</objective>
...
<context>
@cha-bio-safety/docs/redesign-context/26-staff-manage/26-staff-manage.md
...
<task type="auto">
  <name>Task 1: wave-1-index.md 단일 atomic 생성 (8-section + 4 sub-area 인벤토리 + 비즈 anchor + 메모리 룰 inline + OQ + verify gate)</name>
  <files>cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md</files>
  <action>
다음 8-section 구조로 단일 markdown 1개를 생성한다. 24-checkpoints W1 (622l) mirror 패턴이되 **StaffManagePage 는 530 lines + admin 전용 + BottomSheet/DesktopModal 분기 + 폼 8 필드 + ReplaceModal + confirmReset + confirmDeactivate** 로 24-checkpoints(693l) 보다 단순 (19 카테고리 / 유도등 (FPM-) / 소화기 자동 매핑 / 'basement'='common' eq 분기 없음) → §1 인벤토리는 4 sub-area, sub-wave 는 3 sub-wave (W2~W4) + W5 TSX.

**Read 보강 권장:** 실제 source 의 라인 정확도가 핵심. atomic task 진입 시점에 다음을 추가 Read 권장 (이미 brief 시점에 plan author 가 한 번 봤지만, executor 가 다시 확인):
- `cha-bio-safety/src/pages/StaffManagePage.tsx` 1~530 라인 전체 (line 박스 박제 검증)
- `cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md` 622 라인 전체 (§3 fence / §5 메모리 룰 / §6 negative / §7 OQ / §8 verify gate 구조 mirror)
- `cha-bio-safety/docs/redesign-context/26-staff-manage/design-system.md` §1.1 / §1.2 / §1.3 / §6.4 / §6.5 / §6.8 / §7.1 7 fence verbatim 추출

---

**YAML frontmatter** (top, file 첫 줄):
...
머리말 (5~7줄): 단일 파일 530 lines, 4 sub-area (1. 외곽+admin 가드 (imports / 인라인 SVG IconUserPlus / `<style>` keyframes) / 2. 헤더 (데스크톱: 직원 관리 타이틀 + 카운트 + 직원 추가 버튼 / 모바일: 카운트 only) / 3. 콘텐츠 (모바일 카드 + 데스크톱 테이블 7 컬럼 + skeleton/error/empty + 모바일 FAB) / 4. BottomSheet+DesktopModal + StaffModalContent 등록 폼 (8 필드 + role toggle + confirmReset + confirmDeactivate) + ReplaceModalContent (교체 모달, 2× staffApi.update 시퀀스) — 통합 sub-area) 로 정리. line 범위는 실측 (Read 검증, 24-checkpoints W1 §1 동일 방식).
...
| 1. 외곽 + admin 가드 | IconUserPlus SVG (size+color prop) | 11~20 | 인라인 SVG 아이콘 | viewBox 24x24 / strokeWidth 2 | W5 (Lucide UserPlus 치환 검토 — OQ #6) |
...
| 1. 외곽 + admin 가드 | StaffFormState + EMPTY_STAFF_FORM (10 키, 단 폼 UI 는 8 필드) | 70~78 | 폼 state 타입 | shiftOffset+shiftFixed 는 UI 미노출 (ReplaceModal 에서만 변경) | 보존만 |
| 1. 외곽 + admin 가드 | SKELETON_STYLE (`bg var(--bg3) / radius 12 / height 64 / blink 2s ease-in-out infinite`) | 342~345 | 스켈레톤 스타일 상수 | reusable | W3 |
...
| 1. 외곽 + admin 가드 | window.__openReplaceModal 훅 useEffect (line 365~368) | 365~368 | 교체 모달 cross-trigger | StaffModalContent line 292 에서 호출 | 보존만 (변경 금지) |
...
| 2. 헤더 | 데스크톱 헤더 (`flex / alignItems center / padding 12px 24px / borderBottom var(--bd)`) — "직원 관리" 타이틀 + 카운트 + "직원 추가" 버튼 | 398~408 | 데스크톱 상단 영역 | isDesktop 분기 / `staffList.length`명 / mode='add' modal 트리거 | W2 |
...
| 3. 콘텐츠 (목록) | 상태 dot (테이블 line 470~476) — 활성 var(--safe) / 비활성 var(--t3), fontSize 11 | 469~476 | 데스크톱 상태 표시 | s.active !== 0 분기 + 라벨 '활성' / '비활성' | W3 |
| 3. 콘텐츠 (목록) | 모바일 카드 리스트 (`StaffCard` map, line 318~339 정의 사용) | 489~501 | 모바일 카드 리스트 | staffList.map / onClick edit | W3 |
| 3. 콘텐츠 (목록) | 모바일 카드 empty (`'등록된 직원이 없습니다' + '직원 추가 버튼을 눌러 첫 번째 직원을 등록하세요'`) | 491~495 | empty + 안내 | staffList.length === 0 / 60px 16px padding | W3 |
| 3. 콘텐츠 (목록) | StaffCard (모바일 카드) — 8x8 dot bg var(--safe)/var(--t3) + name + title + role 배지 (9px) + id JetBrains Mono + '수정 ▸' | 318~339 | 모바일 카드 컴포넌트 | onClick edit / s.active===0 opacity 0.5 | W3 |
| 3. 콘텐츠 (목록) | 모바일 FAB (`sticky bottom + padding 0 16 + bg var(--acl) + IconUserPlus 18 + '직원 추가' + paddingBottom calc(16+var(--sab)) + height 52`) | 505~513 | mode='add' modal 트리거 | onClick setModal({open,mode:'add'}) / sab safe-area-inset-bottom | W3 |
...
| 4. 모달 wrapper + 폼 | StaffModalContent useState 2건 (confirmReset / confirmDeactivate) + form state (mode==='edit' 시 staff 데이터로 초기화) | 151~161 | 모달 본문 state | shiftOffset 숫자→문자열 / shiftFixed null→'' | 보존만 |
| 4. 모달 wrapper + 폼 | createMutation (staffApi.create / 사번 검증 / toast '직원이 추가되었습니다') | 166~170 | 등록 비즈 로직 | onSuccess invalidate ['staff-list'] + toast | 보존만 (1 byte 변경 금지) |
| 4. 모달 wrapper + 폼 | updateMutation (staffApi.update(id, payload)) | 172~176 | 수정 비즈 로직 | onSuccess invalidate ['staff-list'] + toast '직원 정보가 수정되었습니다' | 보존만 (1 byte 변경 금지) |
| 4. 모달 wrapper + 폼 | resetPwMutation (staffApi.resetPassword) | 178~182 | 비밀번호 초기화 비즈 로직 | onSuccess toast '비밀번호가 초기화되었습니다 (사번 뒷 4자리)' + setConfirmReset(false) | 보존만 (1 byte 변경 금지) |
| 4. 모달 wrapper + 폼 | deactivateMutation (staffApi.update(id, { active: 0 })) | 184~188 | 비활성화 비즈 로직 | onSuccess invalidate ['staff-list'] + toast '직원이 비활성화되었습니다' | 보존만 (1 byte 변경 금지) |
| 4. 모달 wrapper + 폼 | canSave + handleSave + 사번 10자리 정규식 + mode 분기 | 190~200 | 액션 핸들러 | form.name.trim() !== '' && form.id.trim() !== '' / `/^\d{10}$/.test(form.id)` | 보존만 |
...
| 4. 모달 wrapper + 폼 | 폼 필드 8 — 역할 toggle button row (admin / assistant — 라벨 '관리자' / '보조자', height 36 + fontSize 12 + border 1px solid var(--bd)) | 243~253 | 폼 필드 8 (role toggle) | onClick setForm 토글 / `flex 1 / height 36 / fontSize 12` | W4 (OQ #3 toggle UI 변경 검토) |
| 4. 모달 wrapper + 폼 | confirmReset 영역 (edit 모드 only) — 토글 링크 ('비밀번호 초기화', var(--warn) underline) → 확인 박스 (rgba(245,158,11,.08) + '사번 뒷 4자리로 비밀번호를 초기화합니다. 계속하시겠습니까?' + 취소/초기화 버튼 height 32) | 255~274 | 비밀번호 초기화 confirm | mode === 'edit' / setConfirmReset(true/false) / resetPwMutation.mutate() | W4 (verbatim 보존) |
| 4. 모달 wrapper + 폼 | 액션 영역 (취소 / 저장 button 분할, height 44, edit 시 비활성화 / 교체 버튼 row 추가 (height 40) + confirmDeactivate 시 빨간 안내 + 빨간 비활성화 버튼) | 277~312 | 액션 row | canSave 계산 / isBusy / 비활성화 카피 / 교체 onClick → `window.__openReplaceModal?.(staff)` 100ms 지연 | W4 (verbatim 보존) |
| 4. 모달 wrapper + 폼 | ReplaceModalContent (교체 모달) — useQuery candidates (active===1 && shiftOffset===null && shiftFixed===null) / shiftLabel ('평일 주간 고정' / `3교대 (오프셋 N)` / '미설정') / 2× staffApi.update 시퀀스 (1. 신규에 이전 / 2. 기존 비활성화 + phone/email 제거 + shiftOffset/Fixed null) + select + 취소/교체 버튼 | 81~149 | 교체 모달 (별도 sub-area 통합) | window.__openReplaceModal 호출 시 진입 / toast `${oldStaff.name} → ${newStaff.name} 교체 완료` | W4 (verbatim 보존, 인라인 박제) |
| 4. 모달 wrapper + 폼 | 모달 호출 wrapper (`{modal.open && <ModalWrapper ...><StaffModalContent .../></ModalWrapper>}` + `{replaceModal.open && replaceModal.target && <ModalWrapper ...><ReplaceModalContent .../></ModalWrapper>}`) | 516~527 | 모달 진입 | modal.open / replaceModal.open | W4 |
...
- '비활성화' (mode==='edit' && !active===0 row + confirmDeactivate panel, line 290, 308)
- '교체' (mode==='edit' && !active===0 row, line 294)
...
- input/select bg `var(--bg3)` → `var(--surface-sunken)` 치환 OK / StaffCard 배경에도 동일
...
- danger `var(--danger)` → `--status-danger` 치환 OK (별 표시 + 비활성화 버튼 + confirmDeactivate)
- safe `var(--safe)` → `--status-safe-bar` 치환 OK (활성 dot + '활성' 라벨)
- warn `var(--warn)` (확인 안 됨 — tokens.css 의 warning 토큰일 가능성) → `--status-warning-bar` 치환 검토 (비밀번호 초기화 토글 링크 line 258 + confirm 박스 + 교체 버튼 + 초기화 버튼)
...
- 비활성화 안내 박스 `rgba(239,68,68,.08)` → status-danger-bg 알리아스 치환 OK (line 301)
- confirmReset 안내 박스 `rgba(245,158,11,.08)` → status-warning-bg 알리아스 치환 OK (line 262)
- 교체 버튼 `rgba(245,158,11,.1)` + `#d97706` → status-warning + 알리아스 치환 검토 (line 293)
- ReplaceModal info 박스 `rgba(59,130,246,.08)` → status-info-bg 알리아스 치환 OK (line 118)
- SVG IconUserPlus → Lucide `UserPlus` 치환 검토 (OQ #6, size 16/18)
- duty 토큰 매핑 — 현재 StaffManagePage 는 duty 색 미사용 (출근부 페이지에서만). 카드에 근무 패턴 표시 검토 → 26-staff-manage.md §4 "직원 카드의 근무 패턴 표시는 duty 토큰 색 사용 (출근부와 동일 색)" → 신규 디자인 검토 영역 (OQ #3 + 별도 OQ "duty 색 표시 신규?")
```
...
| W3 | `sketch-wave-3-list-fab.html` | 콘텐츠 (모바일 카드 리스트 + 데스크톱 테이블 7 컬럼 + skeleton/error/empty + 모바일 FAB) | sketch HTML | 240~340 | StaffCard 8x8 dot+role 배지+'수정 ▸' / 테이블 7 컬럼 / role 배지 admin/assistant / 상태 dot 활성/비활성 / SKELETON / empty 카피 verbatim / FAB IconUserPlus 18 + sab |
...
| 항목 | 룰 |
|---|---|
| 비즈 anchor §1.3 전체 | 1 byte 변경 금지 (admin 가드 / staffApi 4종 호출 / useMutation 4건 / ReplaceModal 2× update 시퀀스 / queryKey ['staff-list'] / rankOfTitle 직급 정렬 / 사번 10자리 정규식 / appointedAt 자동 채움 / shiftOffset+shiftFixed 비즈 데이터 / window.__openReplaceModal 훅 / role 옵션 / 카피 verbatim / placeholder 7건 / toast 8건) |
| 인라인 `style={{...}}` 제거 | Tailwind utility 치환 (26-staff-manage.md §4 요구사항) — 단, BottomSheet slideUp animation + handle bar 등 키프레임 의존 영역은 className + `<style>` 그대로 유지 또는 `tailwind.config` extend |
| `lg:*` prefix | 레이아웃 차이 (모바일 카드 / 데스크톱 테이블 / 모바일 카운트 only / 데스크톱 헤더 한 줄) 에만 사용 — spacing 분기는 토큰 자동 |
| 모달 chrome | 02+06 chrome 룰 직접 적용 X. BottomSheet/DesktopModal 은 별도 패턴 — CheckpointsPage(24) 와 공통화는 별도 task (이 wave 범위 밖, OQ #1) |
| 외곽 hex 변경 | `var(--bg)/--bg2/--bg3/--bg4/--bd/--bd2/--t1/--t2/--t3/--acl/--safe/--danger/--warn` → `--surface-page/--raised/--sunken/--active/--border-default/--border-strong/--text-primary/--secondary/--tertiary/--accent-primary/--status-safe-bar/--status-danger/--status-warning-bar` 치환 OK (design-system.md §4.1 마이그레이션 표) |
| 폰트 격상 | fontSize 9 (role 배지 line 327) 사용 금지 — `text-caption` (12px) 격상 검토 (OQ #5) / fontSize 10 (테이블 role 배지 line 461 / 입사일·생년월일 보조 라벨 line 224/236) / 11 (테이블 status line 471 / 모바일 카운트 X — 12) → `text-caption` 격상 검토 / 12 → `text-caption` / 13 → `text-label` / 14 → `text-body-sm` / 16 → `text-body` (§1.1 노안 친화) |
| 폰트 위계 모달 | 모달 title fontSize 16 → `text-body` 또는 `text-title` 격상 검토 |
| Lucide 치환 | IconUserPlus SVG → `<UserPlus size={16/18} />` (헤더 데스크톱 16 / 모바일 FAB 18) (OQ #6) |
...
(메타) — StaffManagePage 는 그라데이션 사용 없음. 저장 버튼 / 직원 추가 버튼 모두 단색 `var(--acl)`. 교체 버튼은 단색 `#f59e0b` (ReplaceModal) / `rgba(245,158,11,.1)+#d97706` (StaffModal 액션). §6.4 룰 일치 — 그라데이션 신규 도입 금지 (OQ #2 'CTA 버튼 §6.4 그라데이션 적용?' default 답 = 미적용).
...
(메타) — StaffManagePage 의 IconUserPlus (line 11~20, 16/18px) 인라인 SVG → Lucide `<UserPlus />` 치환 (OQ #6). 데스크톱 헤더 16 / 모바일 FAB 18 → 16 또는 20 (§7.1 사이즈 룰).
...
| `.text-status-safe-bar` / `.text-status-danger` / `.text-status-warning-bar` | tokens.css | 활성 dot+'활성' / 비활성화 버튼+confirmDeactivate / 비밀번호 초기화 링크+confirmReset+교체 버튼 |
...
8. **`feedback_tailwind_w8_h8_is_48px`** — tailwind.config spacing override: `w-8` = 48 (기본 32 아님), `w-7` = 32. 26-staff-manage 의 8x8 dot (StaffCard line 321 / 테이블 line 474) / handle bar 32x4 (line 32) / role 배지 / SKELETON 64 height / FAB height 52 모두 arbitrary `w-[Npx]` 또는 utility 정확 매핑.
9. **`feedback_text_caption_leading_none`** — 작은 컨테이너 안 `text-caption` lh:1.5 (18px) 가 `h-8` (32px) 안에서도 시각적 패딩. role 배지 (height 작음) / 모바일 카드 액션 '수정 ▸' / 테이블 status / 카운트 라벨 / 입사일·생년월일 보조 라벨 모두 `leading-none` 명시 검토.
...
11. **`project_redesign_15_daily_report_status`** — 캘리브 좌표 시스템 100% 보존 패턴 일반화 → 26-staff-manage 의 비즈 anchor (admin 가드 / staffApi 4종 / useMutation 4 / ReplaceModal 2× update 시퀀스 / queryKey ['staff-list'] / rankOfTitle / 사번 10자리 정규식 / appointedAt 자동 채움 / shiftOffset+shiftFixed 데이터 / window.__openReplaceModal 훅 / role 옵션 / 카피 verbatim) 1 byte 변경 금지.
...
16. **평면(flat) 폴더 룰 위반 0** — sketch HTML 은 (다음 wave 에서) `cha-bio-safety/docs/redesign-context/26-staff-manage/sketch-wave-N-{slug}.html` 평면 배치. `sketch/` 서브폴더 만들기 금지. 28-splash / 23-education / 25-qr-print / 24-checkpoints 동일 패턴.
...
## OQ #3 — shiftOffset / shiftFixed 폼 UI 노출 검토 + role toggle UI 변경 (button row → segment control)
...
**문제 2 (role toggle):** 등록 폼 role 선택은 2 button row (`flex 1 / height 36 / fontSize 12`) 토글 패턴 (line 245~252). fontSize 12 는 §1.1 통과. height 36 은 §1.1 모바일 44px 미달 → height 40~44 격상 검토.

**옵션:**
- (a) shift 폼 미노출 유지 (ReplaceModal 만) + role button row 유지 + fontSize 12 → 12 (변경 없음) + height 36 → 40 (데스크톱) / 44 (모바일) 격상
- (b) shift 폼 신규 노출 (shiftOffset 0~3 select + shiftFixed 'day'/null toggle 또는 select) + role 동일
- (c) 둘 다 segment control (한 줄 캡슐) 로 변경

**Default:** (a) — 시각 + 인터랙션 변경 최소. role height 만 §1.1 준수 격상. shift 폼 노출은 별도 task (26 의 운영 룰 영향 → 메모리 `project_meal_calc_rules` + `project_div_compressor_pair` 검토 후).
...
**옵션:**
- (a) 모두 새 토큰 치환 (`--bg`→`--surface-page` / `--bg2`→`--surface-raised` / `--bg3`→`--surface-sunken` / `--bg4`→`--surface-active` / `--bd`→`--border-default` / `--bd2`→`--border-strong` / `--t1/--t2/--t3`→`--text-primary/--secondary/--tertiary` / `--acl`→`--accent-primary` 또는 `--status-info-bar` / `--safe`→`--status-safe-bar` / `--danger`→`--status-danger` / `--warn`→`--status-warning-bar`)
- (b) 기존 raw 변수 유지
...
- 모바일 카드 메타 (title / id) fontSize 12 (line 325, 334)
- ReplaceModal info 박스 보조 라벨 fontSize 11 (line 121)
- confirmReset 버튼 fontSize 12 (line 265, 267)

**옵션:**
- (a) 모두 `text-caption` (12px) 격상 + 작은 영역은 `leading-none` 추가 (`feedback_text_caption_leading_none`)
- (b) role 배지만 `--radius-pill` + 12px / 테이블 status + 보조 라벨은 데스크톱 13px / 모바일 12px (자동 분기) 격상
...
**문제 2 (duty):** 26-staff-manage.md §4 "직원 카드의 근무 패턴 표시는 duty 토큰 색 사용 (출근부와 동일 색)" → 현재 StaffCard / 테이블 row 에 shiftOffset+shiftFixed 시각 표시 없음. 신규 디자인 도입 여부 (예: 카드 우측 dot bar / 테이블 컬럼 추가).

**옵션:**
- (a) `<UserPlus size={16} />` (헤더 데스크톱 line 405) + `<UserPlus size={18} />` (모바일 FAB line 509) — 기존 size 보존 + duty 색 신규 미도입
- (b) 모든 UserPlus size={16} 으로 통일 (§7.1 사이즈 룰 "16 / 20 / 24 px 세 종류만") — 18 → 20 격상 또는 16 통일 + duty 색 카드 추가 (별도 시안 합의)
...
# §6 negative rule ≥10건 (번호 + 굵은 글씨 패턴)
...
**최종 sanity check (executor 자체 검수):**

산출 파일 작성 후 `wc -l` 로 라인 수 확인 (450~600 범위), §8 verify gate 8건 모두 직접 실행 (PASS 확인), 비즈 anchor 박스 의 line 번호가 실제 StaffManagePage.tsx 와 일치하는지 sample 3건 (예: line 22~40 BottomSheet / line 81~149 ReplaceModalContent / line 371~373 admin 가드) 직접 Read 비교. drift 발견 시 라인 번호 보정 후 재커밋 (atomic 1개로 합치기).
  </action>
  <verify>
    <automated>
# §1~§8 헤더 grep == 8 (정확)
test "$(grep -c '^# §[1-8]\.' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md)" -eq 8 \
  && echo "[8.1 PASS] §1~§8 헤더 8개" \
  || (echo "[8.1 FAIL]" && exit 1)

# sub-wave row ≥3
test "$(grep -E '^\| W[2-4] \|' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md | grep -v '^#' | wc -l)" -ge 3 \
  && echo "[8.2 PASS] sub-wave row ≥3" \
  || (echo "[8.2 FAIL]" && exit 1)

# unique 메모리 룰 slug ≥10
test "$(grep -oE '(feedback|project|reference)_[a-z0-9_]+' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md | sort -u | wc -l)" -ge 10 \
  && echo "[8.3 PASS] unique slug ≥10" \
  || (echo "[8.3 FAIL]" && exit 1)

# OQ ≥5
test "$(grep -c '^## OQ #' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md)" -ge 5 \
  && echo "[8.4 PASS] OQ ≥5" \
...
# sketch HTML 추가 0
test "$(find cha-bio-safety/docs/redesign-context/26-staff-manage -name 'sketch-wave-*.html' 2>/dev/null | wc -l)" -eq 0 \
...
test "$(grep -c '^## §3\.' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md)" -ge 7 \
  && echo "[FENCE 7 PASS]" \
  || (echo "[FENCE 7 FAIL]" && exit 1)

# negative rule ≥10건
test "$(grep -cE '^[0-9]+\.\s+\*\*' cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md)" -ge 10 \
  && echo "[NEGATIVE ≥10 PASS]" \
  || (echo "[NEGATIVE ≥10 FAIL]" && exit 1)
    </automated>
  </verify>
  <done>
- `cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md` 단일 파일 생성 (450~600 lines)
- §1~§8 8-section 헤더 모두 존재
- §1.3 비즈 anchor 박스 박제 (admin 가드 / staffApi 4종 호출 / useMutation 4건 (create/update/resetPw/deactivate) / ReplaceModalContent 2× update 시퀀스 / queryKey ['staff-list'] / rankOfTitle 직급 정렬 / 사번 10자리 정규식 / appointedAt 자동 채움 / shiftOffset+shiftFixed 비즈 데이터 / window.__openReplaceModal 훅 / role 옵션 / 카피 verbatim + placeholder 7건 + toast 8건 + ReplaceModal 카피)
- §2 sub-wave 3개 (W2 외곽+가드+헤더 / W3 콘텐츠 리스트+테이블+FAB / W4 모달+폼+ReplaceModal) + W5 TSX
- §3 design-system v0.1.1 fence verbatim 7건 (§1.1 / §1.2 / §1.3 / §6.4 / §6.5 / §6.8 / §7.1)
- §5 메모리 룰 inline ≥10 unique slug (+ 26-staff-manage 특화 2건 `project_div_compressor_pair` + `project_meal_calc_rules`)
- §6 negative rule ≥10건
- §7 OQ 6건 (default 답 명시)
- §8 verify gate 8개 (자동화)
- `src/**` 수정 0
- sketch HTML 추가 0
- wrangler / npm run deploy 0
- atomic commit (한글 OK): `docs(26-staff-manage): wave 1 index — StaffManagePage 530l 4-area 인벤토리 + 비즈 anchor + design-system fence 7 + 메모리 룰 ≥10 + OQ ≥5 + verify 8`
  </done>
</task>
...
- §1~§8 8-section 구조 완비, §1.3 비즈 anchor 박스 박제, §3 fence verbatim 7건, §5 메모리 룰 ≥10 slug, §6 negative ≥10, §7 OQ 6, §8 verify gate 8.
- atomic commit 1개 (한글 메시지 OK).
- W2 진입 가능 상태 (OQ 6건 사용자 컨펌 대기).
</verification>

<success_criteria>
- W2~W4 후속 sketch wave + W5 TSX 작업자가 이 단일 인덱스 파일 1개만 읽고 다음을 즉시 알 수 있다:
  - StaffManagePage.tsx 530 lines 의 4 sub-area + line 범위
  - admin 전용 가드 (useEffect + early return) 보존 룰
  - BottomSheet (모바일) + DesktopModal (데스크톱) 분기 룰 (CheckpointsPage 24 와 공통 — 이 wave 는 인라인 박제만)
  - ReplaceModalContent (교체 모달) 2× staffApi.update 시퀀스 + window.__openReplaceModal 훅 룰
  - 비즈 anchor (staffApi 4종 + useMutation 4 + rankOfTitle + 사번 10자리 정규식 + appointedAt 자동 채움 + shiftOffset+shiftFixed 데이터 + role 옵션 + 카피 + placeholder + toast) 1 byte 변경 금지 룰
  - design-system v0.1.1 7 fence verbatim
  - 메모리 룰 12건 + 26 특화 2건 (이 wave 에서 강제되는 모든 룰)
  - negative rule 17건 (이 wave 에서 금지된 것)
  - OQ 6건 (W2 진입 직전 사용자 컨펌 영역)
...
```
docs(26-staff-manage): wave 1 index — StaffManagePage 530l 4-area 인벤토리 + 비즈 anchor + design-system fence 7 + 메모리 룰 ≥10 + OQ ≥5 + verify 8

- cha-bio-safety/docs/redesign-context/26-staff-manage/wave-1-index.md (단일 파일)
- 24-checkpoints W1 mirror (8-section + sub-wave 3 단일 파일 패턴)
