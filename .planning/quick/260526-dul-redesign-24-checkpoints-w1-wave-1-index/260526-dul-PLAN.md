...
autonomous: true
requirements: [Q-24-W1]
must_haves:
  truths:
    - "wave-1-index.md 파일이 단일 진입점으로 존재한다 (cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md)"
    - "§1~§8 8-section 헤더가 모두 존재한다 (인벤토리 / sub-wave / design-system fence / components.css inherit / 메모리 룰 / negative / OQ / verify)"
    - "§1 인벤토리 표가 CheckpointsPage.tsx 693 lines 5 sub-area (외곽+헤더+권한 가드 / 카테고리+필터 선택 / 카드 리스트(모바일)+테이블(데스크톱) / 등록·수정 모달(BottomSheet+DesktopModal) / 등록 폼 필드+소화기 분기) line 범위 박제"
    - "§2 sub-wave 분배표 4 sub-wave (W2 외곽+헤더+권한 / W3 카테고리+필터+카드/테이블 / W4 BottomSheet+DesktopModal 등록 폼 / W5 인터랙션+loading/error/empty + skeleton) + W6 TSX"
    - "§3 design-system v0.1.1 fence verbatim 7건 (§1.1 / §1.2 / §1.3 / §6.4 / §6.5 / §6.8 / §7.1)"
    - "§5 메모리 룰 inline 인용 ≥10 unique slug"
    - "§6 negative rule ≥8건 (src/** 0 / wrangler 0 / npm run deploy 0 / sketch HTML 0 / 비즈 anchor 변경 0 / CATEGORIES_FALLBACK 19종 변경 0 / admin 가드 변경 0 등)"
    - "§7 OQ ≥5 (BottomSheet/DesktopModal 공통화 / 카테고리 카드 grid expand 패턴 적용 여부 / 등록 폼 zone toggle UI / 외곽 hex 토큰 치환 / Lucide IconPlus/ChevronDown 치환)"
    - "§8 verify gate 8개 (§1~§8 헤더 grep / sub-wave row ≥4 / unique slug ≥10 / OQ ≥5 / src/** 0 / sketch HTML 0 / 비즈 anchor 박스 존재 / fence ≥7 / negative ≥8)"
    - "§1.3 비즈 anchor 박스 박제 (CATEGORIES_FALLBACK 19종 / ZONE_LABEL / ZONE_FLOORS / FLOOR_ORDER / MARKER_TYPE_LABEL / FLOOR_CODE / queryKey 6건 / useMutation 3건 (create/update/deactivate) / extinguisherApi.create 자동 매핑 / floorPlanMarkerApi.update+delete / admin 가드 useEffect / toast 카피 verbatim 8건)"
  artifacts:
    - path: "cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md"
      provides: "redesign/24-checkpoints W2~W5 sketch + W6 TSX 진입을 위한 단일 인덱스 (8-section + 비즈 anchor + OQ)"
      min_lines: 500
  key_links:
    - from: "wave-1-index.md §1"
      to: "CheckpointsPage.tsx 693 lines"
      via: "5 sub-area line 범위 박제 + 비즈 anchor 보존"
...
<objective>
redesign/24-checkpoints W1 — `CheckpointsPage.tsx` (693 lines, admin 전용 BottomSheet/DesktopModal 패턴) 의 element 인벤토리 + 비즈 anchor 박제 + design-system v0.1.1 fence verbatim + 메모리 룰 inline + negative rule + OQ + verify gate 를 통합한 **단일 진입점 마크다운** 1개 (`wave-1-index.md`) 를 생성한다. W2~W5 후속 sketch wave + W6 TSX 변환 작업자는 이 파일 하나만 읽으면 모든 컨텍스트를 확보한다.

Purpose: 25-qr-print / 23-education / 28-splash / 10-cctv-info / 27-login W1 인덱스 mirror 패턴을 24-checkpoints 에도 적용. 다음 wave (W2~W5 sketch + W6 TSX) 의 컨텍스트 낭비 차단 + 비즈 anchor (admin 가드 + CATEGORIES_FALLBACK 19종 + queryKey 6건 + useMutation 3건 + 소화기 자동 매핑 + floorPlanMarkerApi 분기 + 유도등 (FPM-) 분기 + ZONE_FLOORS 비즈 데이터) 1 byte 변경 사고 사전 예방.

Output: `cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md` 단일 파일 (500~700 lines 예상).
</objective>
...
<task type="auto">
  <name>Task 1: wave-1-index.md 단일 atomic 생성 (8-section + 5 sub-area 인벤토리 + 비즈 anchor + 메모리 룰 inline + OQ + verify gate)</name>
  <files>cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md</files>
  <action>
다음 8-section 구조로 단일 markdown 1개를 생성한다. 25-qr-print/23-education/28-splash W1 mirror 패턴이되 **CheckpointsPage 는 693 lines + admin 전용 + BottomSheet/DesktopModal 분기 + 19 카테고리 + 등록 폼 + 소화기 자동 매핑 + 유도등 (FPM-) 분기** 로 25-qr-print(330l) 보다 복잡 → §1 인벤토리는 5 sub-area, sub-wave 는 4 sub-wave (W2~W5) + W6 TSX.
...
머리말 (5~7줄): 단일 파일 693 lines, 5 sub-area (1. 외곽+권한 가드 / 2. 헤더(카테고리+필터 select) (모바일/데스크톱 분기) / 3. 콘텐츠 (모바일 카드 리스트 + 모바일 FAB / 데스크톱 테이블) + skeleton+loading+error+empty / 4. 모달 wrapper (BottomSheet vs DesktopModal) + CheckPointModalContent / 5. CheckPointModalContent 등록 폼 필드 + 소화기 (isExtCategory) 분기 + 비활성화 confirm) 로 정리. line 범위는 실측 (Read 검증).
...
# §6 negative rule ≥8건 (번호 + 굵은 글씨 패턴)
grep -cE "^[0-9]+\.\s+\*\*" cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md
# 기대: ≥8
```
...
- **v1** (2026-05-26, quick `260526-dul`) — redesign/24-checkpoints W1 인덱스 최초 생성. 25-qr-print / 23-education / 28-splash / 10-cctv-info / 27-login W1 mirror (8-section + sub-wave 4 + 비즈 anchor + 메모리 룰 12 + OQ 6 + verify 8). 단일 파일 (CheckpointsPage.tsx 693 lines, admin 전용 BottomSheet/DesktopModal) 5 sub-area 패턴.
  </action>
  <verify>
    <automated>
# §1~§8 헤더 grep == 8 (정확)
test "$(grep -c '^# §[1-8]\.' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md)" -eq 8 \
  && echo "[8.1 PASS] §1~§8 헤더 8개" \
  || (echo "[8.1 FAIL]" && exit 1)

# sub-wave row ≥4
test "$(grep -E '^\| W[2-5] \|' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md | grep -v '^#' | wc -l)" -ge 4 \
  && echo "[8.2 PASS] sub-wave row ≥4" \
  || (echo "[8.2 FAIL]" && exit 1)

# unique 메모리 룰 slug ≥10
test "$(grep -oE '(feedback|project|reference)_[a-z0-9_]+' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md | sort -u | wc -l)" -ge 10 \
  && echo "[8.3 PASS] unique slug ≥10" \
  || (echo "[8.3 FAIL]" && exit 1)

# OQ ≥5
test "$(grep -c '^## OQ #' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md)" -ge 5 \
  && echo "[8.4 PASS] OQ ≥5" \
...
# sketch HTML 추가 0
test "$(find cha-bio-safety/docs/redesign-context/24-checkpoints -name 'sketch-wave-*.html' 2>/dev/null | wc -l)" -eq 0 \
...
test "$(grep -c '^## §3\.' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md)" -ge 7 \
  && echo "[FENCE 7 PASS]" \
  || (echo "[FENCE 7 FAIL]" && exit 1)

# negative rule ≥8건
test "$(grep -cE '^[0-9]+\.\s+\*\*' cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md)" -ge 8 \
  && echo "[NEGATIVE ≥8 PASS]" \
  || (echo "[NEGATIVE ≥8 FAIL]" && exit 1)
    </automated>
  </verify>
  <done>
- `cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md` 단일 파일 생성 (500~700 lines)
- §1~§8 8-section 헤더 모두 존재
- §1.3 비즈 anchor 박스 박제 (CATEGORIES_FALLBACK 19종 / ZONE_LABEL / ZONE_FLOORS / FLOOR_ORDER 20건 / MARKER_TYPE_LABEL 6건 / FLOOR_CODE 8건 / admin 가드 / queryKey 6 / useMutation 3 / 소화기 자동 매핑 / 유도등 FPM- 분기 / 'basement'='common' eq / canSave / 카피 verbatim 8건 + placeholder 9건 + toast 7건)
- §2 sub-wave 4개 (W2 외곽+가드 / W3 헤더+필터 / W4 콘텐츠 리스트+FAB / W5 모달+폼) + W6 TSX
- §3 design-system v0.1.1 fence verbatim 7건 (§1.1 / §1.2 / §1.3 / §6.4 / §6.5 / §6.8 / §7.1)
...
- atomic commit (한글 OK): `docs(24-checkpoints): wave 1 index — CheckpointsPage 693l 5-area 인벤토리 + 비즈 anchor + design-system fence 7 + 메모리 룰 ≥10 + OQ ≥5 + verify 8`
  </done>
</task>
...
- §1~§8 8-section 구조 완비, §1.3 비즈 anchor 박스 박제, §3 fence verbatim 7건, §5 메모리 룰 ≥10 slug, §6 negative ≥8, §7 OQ 6, §8 verify gate 8.
- atomic commit 1개 (한글 메시지 OK).
- W2 진입 가능 상태 (OQ 6건 사용자 컨펌 대기).
</verification>

<success_criteria>
- W2~W5 후속 sketch wave + W6 TSX 작업자가 이 단일 인덱스 파일 1개만 읽고 다음을 즉시 알 수 있다:
  - CheckpointsPage.tsx 693 lines 의 5 sub-area + line 범위
  - admin 전용 가드 (useEffect + early return) 보존 룰
  - BottomSheet (모바일) + DesktopModal (데스크톱) 분기 룰 (StaffManagePage 26 와 공통 — 이 wave 는 인라인 박제만)
  - 비즈 anchor (CATEGORIES_FALLBACK 19종 + ZONE_FLOORS + FLOOR_ORDER + MARKER_TYPE_LABEL + FLOOR_CODE + queryKey 6 + useMutation 3 + 유도등 FPM- 분기 + 소화기 자동 매핑 + 'basement'='common' eq + canSave + 카피 + placeholder + toast) 1 byte 변경 금지 룰
...
```
docs(24-checkpoints): wave 1 index — CheckpointsPage 693l 5-area 인벤토리 + 비즈 anchor + design-system fence 7 + 메모리 룰 ≥10 + OQ ≥5 + verify 8

- cha-bio-safety/docs/redesign-context/24-checkpoints/wave-1-index.md (단일 파일)
- 25-qr-print / 23-education / 28-splash / 10-cctv-info W1 mirror (8-section + sub-wave 4 단일 파일 패턴)
- W2~W5 sketch + W6 TSX 진입 가능
```
