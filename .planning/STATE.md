...
Last activity: 2026-05-15 - Completed quick task 260515-iz1: redesign/07-elevator TSX Wave 3 — Fault 모달 3종 (FaultNewModal + FaultNewFullscreen + FaultResolveModal) v0.1.1 토큰 + Tailwind 변환 (2B 시안 권위). 이모지 (🚨/✕) → AlertTriangle/X lucide. FaultNew CTA = fire 그라디언트 (§6.4 예외) / FaultResolve CTA = accent 단색. TYPE_ICON_COMPONENT 매퍼 재사용 (Wave 1 신설). 풀스크린 화이트리스트 (position/zIndex/paddingTop+safe-area). TKE 자동 다이얼 보존. 비즈니스 로직 100% 보존, 인라인 금지 키 0건 / 9-11px 0건 / 이모지 0건. npm build PASS. 다음: Wave 4 (RepairNewModal, 2C 시안).
