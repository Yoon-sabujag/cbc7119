...
Last activity: 2026-05-16 - **redesign/04-remediation sketch — RemediationPage v0.1.1 시안 HTML**: remediation-sketch.html 1504줄, 8 viewport (모바일 4 + 데스크톱 2 + 라이트 2). 카드 좌측 색바 = 상태 기준(미조치 fire / 완료 safe, 옛 결과 기준 폐기 — feedback_inspection_unresolved_color 정합) + 결과 배지 = result 기준 토큰 페어(불량 danger / 주의 warning) + 보고서 다운로드 단색 accent(옛 그라디언트 폐기) + 데스크톱 50/50 분할 + 우측 빈 슬롯 가이드. lucide 14건. [data-theme] 18. 옛 토큰 0 / linear-gradient 0 / 이모지 0 / fontSize 11px 1건(TODO). 코드 0건 변경(RemediationPage 567줄/PhotoGrid/ui index 단 한 줄 수정 X). 사용자 확인완료. 다음: TSX 변환 (별도 quick).

Previous: 2026-05-16 - **redesign/03-qr-scan TSX Wave 1 — QRScanPage.tsx 변환**: 297→278줄(-19). qr-scan-sketch.html(00858ae) 1:1 매핑. 이모지 3종 lucide 치환. primary 그라디언트→bg-accent 단색. 글로벌 상수 3종 정의 제거. 비즈니스 로직 100% 보존. TypeScript 0 / npm build PASS. 사용자 검수 → main 머지+배포.
