## 응답 언어 (필수 · 기본 동작 override)

- **모든 응답 본문을 한국어로 작성한다.** 영어 제목/섹션명/기술용어 혼용 금지 — 코드 심볼·식별자·파일 경로·명령어만 원문 유지.
- **도구 호출의 `description`(진행 상황 표시 문구)도 한국어로 작성한다.**
- 이 규칙은 세션 시작 및 `/clear` 직후 첫 턴부터 즉시 적용된다. (화면 하단 스피너 문구 "Thinking…" 등은 Claude Code 앱 자체 UI라 제외 — 지시로 변경 불가.)
